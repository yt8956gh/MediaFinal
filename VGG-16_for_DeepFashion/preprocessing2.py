...
def DataPreprocessing():
    ...
    for i in range(len(filename_list)):
        ...
        if set_category[i][1]=='train':
            ...
            print(str(num_entry)+':\t'+set_category[i][1]+'->train:\t'+img_category[i][1]+':\t'+filepath)
        else:
            test_label.append(int(img_category[i][1]))
            print(str(num_entry)+':\t'+set_category[i][1]+'->test:\t'+img_category[i][1]+':\t'+filepath)
    ...
    train_label = utils.to_categorical(train_label, num_classes=50)
    test_label = utils.to_categorical(test_label, num_classes=50)
